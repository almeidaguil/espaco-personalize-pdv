...
test("admin creates an event and opens a cash session for it", async ({
  page,
}) => {
  const uniqueSuffix = crypto.randomUUID();
  const eventName = `Caixa E2E ${uniqueSuffix}`;
  const eventLocation = `Local Caixa E2E ${uniqueSuffix}`;
  ...
  await page.getByLabel("Inicio").fill("2026-07-11T09:00");
  await page.getByLabel("Termino").fill("2026-07-11T18:00");
